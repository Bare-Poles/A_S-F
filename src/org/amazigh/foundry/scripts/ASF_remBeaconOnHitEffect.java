...
public class ASF_remBeaconOnHitEffect implements OnHitEffectPlugin {
	...
	public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target,
		...
		if (target != null) {
			if (target instanceof ShipAPI) {
				...
				float dam = projectile.getDamageAmount() * 0.4f; //20
				...
				if (listener.recentHits.getItems().size() >= 8) {
					...
					String uniqueHash = (String.valueOf(projectile.hashCode()) + String.valueOf((int) engine.getTotalElapsedTime(true)));
					// generating a unique identifying int for each projectile
